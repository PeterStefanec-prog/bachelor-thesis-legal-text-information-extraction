...
> **File:** `src/ingestion/data_cleaner.py`
...
**Note:** pdfplumber sometimes puts multiple paragraphs on one line when the PDF doesn't have explicit paragraph breaks. This can't be fixed in the cleaner – its a limitation of PDF extraction. More in [Known Limits](#known-limits).
...
**Note:** `court` and `document_type` were added later – originally i only extracted the first 3 fields. Easy to add because the header format is standardized across all courts.
...
**Bug i fixed:** Original regex was `\-?\s*\d+\s*\-?` – the dashes were optional (`?`). This means a bare number like `3` or `12` on its own line would also get deleted. I fixed this by making dashes **required**: `^\-\s*\d+\s*\-$`. Now standalone digits (like paragraph numbering `1.`, `2.`) are safe.

#### `split_header_body`
Looks for regex `\nROZSUDOK\n` or `\nUZNESENIE\n` and splits text there into:
- `header_text` – metadata block (court name, case id, ECLI...)
...
for each line:
    ...
    B) NEVER JOIN (STOP):
        ...
        - line starts with list item (1., a), •)
...
**Important:** i preserve paragraph breaks `\n\n`! At the end i join with `"\n\n".join(...)`.

> Originally i was joining with `"\n"` and it looked fine when reading the .txt files. But then the chunker couldn't find paragraph boundaries and was making bad chunks. Fixed by switching to `"\n\n"`.
...
```
'I. Dovolanie o d m i e t a.'  →  'I. Dovolanie odmiet a.'   ← BUG
```

The last `a` is followed by `.` which is `\S`, so `(?!\S)` failed and regex stopped one letter too early.

Fixed `(?!\S)` to `(?=[\s.,;:!?()\[\]]|$)` – meaning "after the word there can be whitespace, punctuation, or end of string". After the fix:
```
'I. Dovolanie o d m i e t a.'  →  'I. Dovolanie odmieta.'    ✓
```

**Note:** I kept the threshold at `{2,}` (at least 2 repetitions of letter+space) on purpose. Lowering it to `{1,}` would also fix two-letter spaced words like `m á`, but the risk of accidental false positives on normal text felt too high so i left it conservative.
...
This runs **after** layout normalization, not before, because during line joining i need to be able to tell the difference between a dash in a sentence vs a word-splitting hyphen at end of line.
...
| Segment | Keyword | Regex |
|---------|---------|-------|
| `verdict` | — | text before `odôvodnenie` |
...
| 5 | `remove_pagination` | `Pokračovanie` was not removed despite being in spec | Added new condition |

---

## Known limits

**pdfplumber merges paragraphs onto one line.**
Some older or scan-to-PDF documents don't have paragraph structure embedded in the PDF. In that case pdfplumber returns multiple paragraphs on a single line. The cleaner can't do anything about this – it doesn't have access to pixel-level layout information. In practice i saw this as merged paragraphs in one decision (paragraphs 35 and 36 ended up joined). I will monitor how often this happens in the corpus and if needed switch to a more robust extractor.
