...
Originally the script used relative path `../../data` which only worked from the `src/ingestion/` folder. Changed to `data` so it works when running from project root:

```bash
.venv/bin/python src/ingestion/data_cleaner.py
```
