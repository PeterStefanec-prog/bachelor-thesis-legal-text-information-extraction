...
# INPUT - data/01_raw_pdfs/{spis}/{KS_x}_{spis}_00_dokument.pdf
# OUTPUT - data/02_processed_json/{KS_x}_{spis}_00_dokument.json
...
# #### STEP 3.2: Light unicode cleaning ####
...
# #### STEP 3.4: Remove pagination and separate main header ####
def remove_pagination(text: str) -> str:
    ...
    for line in lines:
        ...
        if re.match(r"^strana\s+\d+(?:\s*z\s*\d+)?$", s, re.IGNORECASE):    # (?: ... )? non mandatory part
            continue
        # FIX: Now requires actual dashes, so lone '3' or '12' won't get removed    e.g.      - 3 -
...
def split_header_body(text: str):
    # Regex to find  start of  judgment body
    # Usually starts with "ROZSUDOK" or "UZNESENIE" in uppercase on a new line     - before is header and after is body
    ...
    # Fallback to ECLI position - fallback after ECLI line
    ecli_match = re.search(r"ECLI:[^\n]+\n", text)
    if ecli_match:
        return text[:ecli_match.end()].strip(), text[ecli_match.end():].strip() # header until ECLI

    return text[:1000], text[1000:]  # Last resort fallback - 1000 chars from beggining of doc
...
def normalize_layout(text: str) -> str:
    ...
    for line in lines:
        ...
        if prev.endswith("-"):  # Hyphen
            buf = prev[:-1] + line     # merge without whitespace
            continue
        elif re.search(PREPOSITIONS, prev, re.IGNORECASE):  # preposition
            should_join = True
        elif re.search(rf"{ABBREVIATIONS}$", prev, re.IGNORECASE):  # abbrevat - merge
            should_join = True
        ...
        # B) STOP RULES  - do not merge line
            ...
            should_join = False
        elif prev.strip().upper().endswith("SLOVENSKEJ REPUBLIKY"): # previous line is slovenskej repubiky
            should_join = False
    ...
    if buf:
        merged.append(buf)  # in the end add what has left
    return fix_spaced_words("\n\n".join([m for m in merged if m.strip()]))
    #     i found a small bug in my text preprocessing.
    #     Originally in notebook, i joined the paragraphs using a single newline (\n).
...
def advanced_unicode(text: str) -> str:
    ...
    # quote \u201d (U+201D) when i saved the file.
    # That meant ALL THREe replace() calls were producing \u201d instead of ASCII “.
    # So the text still had smart quotes after “normalization” which broke golden quote matching.
    # Using \u escapes avoids this editor problem completely.
    ...
    # quotes, making them NOT exact substrings of the original text.
    # For example  LLM would write “potvrdzuje.” but the text had “potvrdzuje .”
    # so grounding check flagged it as TRUNCATED even though the quote was
    # correct.
    # Easier to fix here in preprocessing than to handle in extraction.
    t = re.sub(r'\s+([.,;:!?\)])', r'\1', t)    #  "potvrdzuje ."  changes to "potvrdzuje."
