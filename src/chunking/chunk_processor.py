...
from langchain_text_splitters import RecursiveCharacterTextSplitter # splitter from langchain (python framework for
    # loading docs, splitting text on chunks, embeddings, searching in vector DB, connecting to LLM, RAG pipeline,...)
    # recurstive splitting test - keep natural text 'borders'
...
# PARAGRAPH separators - only used as SAFETY NET for splitting extremely long
# numbered points that exceeds OpenAIs 8192 token embedding context window.
# the main split is done by split_reasoning_to_paragraphs() which cuts at each
# numbered point boundary (1. 2. 3. ...). this separator list is only needed
# when a single point is so long that it would be truncated by  embedding API.
# in practice, only 8 out of 3581 points (0.2%) in our dataset need this.
...
#
# ###### ME5_200: ######
#   very finegrained chunks - each chunk covers roughly half a legal paragraph
...
#   where each point is one complete legal argument. this is the most natural
#   way to split them - i respect the structure the judge wrote
#
#   PREVIOUS VERSION used RecursiveCharacterTextSplitter with chunk_size=1500
#   which MERGED multiple small points into one chunk (e.g. points 18-25 in one
#   chunk of 1384 tokens). this was wrong because each point has its own topic -
#   merging "plaintiff's appeal" with "court costs" into one chunk creates a mixed
#   embedding that matches neither query well.
#
...
# regex for splitting reasoning into numbered points (1. 2. 3. ...)
# also used by the hierarchical chunking below
...
def split_reasoning_to_paragraphs(reasoning_text):
    ...
    Very long points (>8000 tokens) get further split by the safety-net splitter
    to avoid exceeding OpenAIs 8192 token embedding limit."""
    ...
    if not matches:
        for block in re.split(r"\n\n+", reasoning_text):
            ...
            if len(words) <= 1 and len(clean_block) <= 2:   # filter out things like ".", "," or ":" agter wrong splitting
                continue
            # safety-net split for very long blocks (more than 8000 tokens - much even for openai embedder)
            if openai_token_len(clean_block) > 8000:    # happens almost never
                paragraphs.extend(_para_safety_splitter.split_text(clean_block)) # extend adds each element as single one
            else:
    ...
    if first_start > 0:
        intro_text = reasoning_text[:first_start].strip() # intro text example is - Odôvodnenie odvolacieho súdu
                                                        #1. Súd zistil...
        if intro_text:  # AAGAIN is intro text too long? Probably not - just something like safety net
            if openai_token_len(intro_text) > 8000:
                paragraphs.extend(_para_safety_splitter.split_text(intro_text))
            else:
    ...
    for i, match in enumerate(matches):
        ...
        # safety-net: split only points exceeding OpenAI 8192 context window
        if openai_token_len(text) > 8000:
            paragraphs.extend(_para_safety_splitter.split_text(text))
        else:
...
# --- 5. HELPER FUNCTION  --- JUST FOR FIXED SIZE STRATEGIES!!
def process_chunks(splitter, text, metadata, strategy_name, filename, token_len_functiom):
    raw_chunks = splitter.split_text(text) # splits text into chunks based on params of splitter object of type     raw_chunks = splitter.split_text(text)
...
# ==========================================
# After running the flat chunk experiments above, i noticed that even with
# paragraph-based chunking (OPENAI_PARA), the LLM sometimes gets chopped up
# pieces of legal arguments. The court writes its reasoning as numbered points
# (1. 2. 3. ...) where each point is one complete legal argument. If a point
# is 800 tokens and chunk_size is 500, it gets split into two half-arguments.
#
# Idea: split reasoning into PARENTS (= numbered legal points, natural boundaries)
# and then split each parent into small CHILDREN (~220 tokens) for retrieval.
# Retrieve via children (precise), but send the LLM complete parents (full context) - wanted to do this also with
# 200 tokens, but now implementing formally
#
...
#
# i keep this in  same file because it uses the same tokenizer setup and
# separator logic. It just adds a second output directory with the parent/child split.
...
# chunk_overlap=60 is higher ratio (~27%) because i want children to have
# context from neighboring text within the same parent
...
def split_reasoning_to_parents(reasoning_text):
    """Split reasoning into parent chunks based on numbered points.
    If no numbered points found, fall back to paragraph breaks."""
    # quite a lot of duplicity with function split_reasoning_to_paragraphs(), but for readability its separate
    matches = list(POINT_SPLIT_RE.finditer(reasoning_text))
    parents = []
    ...
    # numbered points found - split at each "number. " boundary - the same logic as in  split_reasoning_to_paragraphs()
    first_start = matches[0].start()
    if first_start > 0:
...
def split_parent_to_children(parent_text):
    """Split a parent into children using the child_splitter.
    Small parents (under 280 tokens) stay whole - i dont want to split
...
def process_hierarchical(doc_data, filename, clean_name):
    ...
    case_id = str(doc_data.get("metadata", {}).get("case_id", "unknown"))
...
def main():
    ...
    # i added this so i can quickly re-chunk after changing the PARA strategy
    # without waiting for all 176 documents to be processed
    if args.golden_only:
        import pandas as pd
